...
class OccupancyGrid:
    def __init__(self, mapXLength, mapYLength, initXY, unitGridSize, lidarFOV, numSamplesPerRev, lidarMaxRange, wallThickness):
        ...
        x = np.linspace(-xNum * unitGridSize / 2, xNum *
                        unitGridSize / 2, num=xNum + 1) + initXY['x']
        y = np.linspace(-yNum * unitGridSize / 2, yNum *
                        unitGridSize / 2, num=yNum + 1) + initXY['y']
        self.OccupancyGridX, self.OccupancyGridY = np.meshgrid(x, y)
        self.occupancyGridVisited = np.ones((yNum + 1, xNum + 1))
        self.occupancyGridTotal = 2 * np.ones((yNum + 1, xNum + 1))
        ...
        radByX, radByY, radByR = self.itemizeSpokesGrid(
            xGrid, yGrid, bearingIdxGrid, rangeIdxGrid)
        self.radByX = radByX
        self.radByY = radByY
        self.radByR = radByR
        ...
        self.spokesStartIdx = int(
            ((self.numSpokes / 2 - self.numSamplesPerRev) / 2) % self.numSpokes)
    ...
    def spokesGrid(self):
        ...
        x = np.linspace(-self.lidarMaxRange,
                        self.lidarMaxRange, 2 * numHalfElem + 1)
        y = np.linspace(-self.lidarMaxRange,
                        self.lidarMaxRange, 2 * numHalfElem + 1)
        ...
        bearingIdxGrid[:, 0: numHalfElem] = np.fliplr(np.flipud(bearingIdxGrid))[
            :, 0: numHalfElem] + int(self.numSpokes / 2)
        bearingIdxGrid[numHalfElem + 1: 2 * numHalfElem +
                       1, numHalfElem] = int(self.numSpokes / 2)
    ...
    def expandOccupancyGridHelper(self, position, axis):
        ...
        self.occupancyGridTotal = np.insert(
            self.occupancyGridTotal, [position], 2 * insertion, axis=axis)
        xv, yv = np.meshgrid(x, y)
        self.OccupancyGridX = np.insert(
            self.OccupancyGridX, [position], xv, axis=axis)
        self.OccupancyGridY = np.insert(
            self.OccupancyGridY, [position], yv, axis=axis)
    ...
    def updateOccupancyGrid(self, reading, dTheta=0, update=True):
        ...
        spokesOffsetIdxByTheta = int(
            np.rint(theta / (2 * np.pi) * self.numSpokes))
        emptyXList, emptyYList, occupiedXList, occupiedYList = [], [], [], []
        for i in range(self.numSamplesPerRev):
            spokeIdx = int(
                np.rint((self.spokesStartIdx + spokesOffsetIdxByTheta + i) % self.numSpokes))
    ...
    def plotOccupancyGrid(self, xRange=None, yRange=None, plotThreshold=True):
        ...
        plt.imshow(ogMap, cmap='gray', extent=[
                   xRange[0], xRange[1], yRange[0], yRange[1]])
        plt.show()
        if plotThreshold:
            ogMap = ogMap >= 0.5
            plt.matshow(ogMap, cmap='gray', extent=[
                        xRange[0], xRange[1], yRange[0], yRange[1]])
            plt.show()
...
def main():
    ...
    for key in sorted(sensorData.keys()):
        ...
        updateTrajectoryPlot(
            sensorData[key], xTrajectory, yTrajectory, colors, count)
        # if count == 100:
        #   break
