...
class ScanMatcher:
    ...
    def frameSearchSpace(self, estimatedX, estimatedY, unitLength, sigma, missMatchProbAtCoarse):
        ...
        idxEndX, idxEndY = int((xRangeList[1] - xRangeList[0]) / unitLength),  int(
            (yRangeList[1] - yRangeList[0]) / unitLength)
        searchSpace = math.log(missMatchProbAtCoarse) * \
            np.ones((idxEndY + 1, idxEndX + 1))

        self.og.checkAndExapndOG(xRangeList, yRangeList)
        xRangeListIdx, yRangeListIdx = self.og.convertRealXYToMapIdx(
        ...
        ogMap = ogMap > 0.5
        ogX = self.og.OccupancyGridX[yRangeListIdx[0]
            : yRangeListIdx[1], xRangeListIdx[0]: xRangeListIdx[1]]
        ogY = self.og.OccupancyGridY[yRangeListIdx[0]
            : yRangeListIdx[1], xRangeListIdx[0]: xRangeListIdx[1]]

        ogX, ogY = ogX[ogMap], ogY[ogMap]
        ogIdx = self.convertXYToSearchSpaceIdx(
            ogX, ogY, xRangeList[0], yRangeList[0], unitLength)
    ...
    def matchScan(self, reading, estMovingDist, estMovingTheta, count, matchMax=True):
        ...
        estimatedX, estimatedY, estimatedTheta, rMeasure = reading[
            'x'], reading['y'], reading['theta'], reading['range']
        rMeasure = np.asarray(rMeasure)
        if count == 1:
            return reading, 1
        # Coarse Search
        # make this even number of unitGridSize for performance
        coarseSearchStep = self.coarseFactor * self.og.unitGridSize
        coarseSigma = self.scanSigmaInNumGrid / self.coarseFactor

        xRangeList, yRangeList, probSP = self.frameSearchSpace(
            estimatedX, estimatedY, coarseSearchStep, coarseSigma, self.missMatchProbAtCoarse)
        ...
        fineMissMatchProbAtFine = self.missMatchProbAtCoarse ** (
            2 / self.coarseFactor)
        xRangeList, yRangeList, probSP = self.frameSearchSpace(
            matchedReading['x'], matchedReading['y'], fineSearchStep, fineSigma, fineMissMatchProbAtFine)
        matchedPx, matchedPy, matchedReading, convTotal, fineConfidence = self.searchToMatch(probSP, matchedReading['x'],
                                                                                             matchedReading['y'], matchedReading['theta'], matchedReading[
                                                                                                 'range'], xRangeList, yRangeList,
    ...
    def searchToMatch(self, probSP, estimatedX, estimatedY, estimatedTheta, rMeasure, xRangeList, yRangeList,
        ...
        px, py = self.covertMeasureToXY(
            estimatedX, estimatedY, estimatedTheta, rMeasure)
        numCellOfSearchRadius = int(searchRadius / unitLength)
        xMovingRange = np.arange(-numCellOfSearchRadius,
                                 numCellOfSearchRadius + 1)
        yMovingRange = np.arange(-numCellOfSearchRadius,
                                 numCellOfSearchRadius + 1)
        xv, yv = np.meshgrid(xMovingRange, yMovingRange)
        if fineSearch:
            rv, thetaWeight = np.zeros(xv.shape), np.zeros(xv.shape)
        else:
            rv = - (1 / (2 * self.moveRSigma ** 2)) * (np.sqrt((xv * unitLength)
                                                               ** 2 + (yv * unitLength) ** 2) - (estMovingDist)) ** 2
            rrv = np.abs(np.sqrt((xv * unitLength) ** 2 +
                         (yv * unitLength) ** 2) - estMovingDist)
            # no points deviates more than maxMoveDeviation
            rv[rrv > self.maxMoveDeviation] = -100
            if estMovingTheta is not None:
                distv = np.sqrt(np.square(xv) + np.square(yv))
                distv[distv == 0] = 0.0001
                thetav = np.arccos(
                    (xv * math.cos(estMovingTheta) + yv * math.sin(estMovingTheta)) / distv)
                thetaWeight = -1 / (2 * self.turnSigma **
                                    2) * np.square(thetav)
            else:
        ...
        thetaRange = np.arange(-searchHalfRad, searchHalfRad +
                               self.og.angularStep, self.og.angularStep)
        convTotal = np.zeros((len(thetaRange), xv.shape[0], xv.shape[1]))
        for i, theta in enumerate(thetaRange):
            rotatedPx, rotatedPy = self.rotate(
                (estimatedX, estimatedY), (px, py), theta)
            rotatedPxIdx, rotatedPyIdx = self.convertXYToSearchSpaceIdx(rotatedPx, rotatedPy, xRangeList[0],
                                                                        yRangeList[0], unitLength)
            uniqueRotatedPxPyIdx = np.unique(
                np.column_stack((rotatedPxIdx, rotatedPyIdx)), axis=0)
            rotatedPxIdx, rotatedPyIdx = uniqueRotatedPxPyIdx[:,
                                                              0], uniqueRotatedPxPyIdx[:, 1]
            #########   For Debug Only  #############
        ...
        if matchMax:
            ...
            convTotalFlattenProb = np.exp(
                convTotalFlatten) / np.exp(convTotalFlatten).sum()
            maxIdx = np.random.choice(
                np.arange(convTotalFlatten.size), 1, p=convTotalFlattenProb)[0]
            maxIdx = np.unravel_index(maxIdx, convTotal.shape)

        confidence = np.sum(np.exp(convTotal))
        dx, dy, dtheta = xMovingRange[maxIdx[2]] * \
            unitLength, yMovingRange[maxIdx[1]] * \
            unitLength, thetaRange[maxIdx[0]]
        matchedReading = {"x": estimatedX + dx, "y": estimatedY + dy, "theta": estimatedTheta + dtheta,
                          "range": rMeasure}
        matchedPx, matchedPy = self.rotate(
            (estimatedX, estimatedY), (px, py), dtheta)
    ...
    def plotMatchOverlay(self, probSP, matchedPx, matchedPy, matchedReading, xRangeList, yRangeList, unitLength):
        ...
        pxIdx, pyIdx = self.convertXYToSearchSpaceIdx(
            matchedPx, matchedPy, xRangeList[0], yRangeList[0], unitLength)
        plt.scatter(pxIdx, pyIdx, c='r', s=5)
        poseXIdx, poseYIdx = self.convertXYToSearchSpaceIdx(
            matchedReading['x'], matchedReading['y'], xRangeList[0], yRangeList[0], unitLength)
...
def updateEstimatedPose(currentRawReading, prevMatchedReading, prevRawReading, prevRawMovingTheta, prevMatchedMovingTheta):
    estimatedTheta = prevMatchedReading['theta'] + \
        currentRawReading['theta'] - prevRawReading['theta']
    estimatedReading = {'x': prevMatchedReading['x'], 'y': prevMatchedReading['y'],
                        'theta': estimatedTheta, 'range': currentRawReading['range']}
    dx, dy = currentRawReading['x'] - \
        prevRawReading['x'], currentRawReading['y'] - prevRawReading['y']
    estMovingDist = math.sqrt(dx**2 + dy**2)
    rawX, rawY, prevRawX, prevRawY = currentRawReading[
        'x'], currentRawReading['y'], prevRawReading['x'], prevRawReading['y']
    ...
    if rawMove > 0.3:
        if prevRawMovingTheta != None:
            if rawYMove > 0:
                rawMovingTheta = math.acos(
                    rawXMove / rawMove)  # between -pi and +pi
            else:
...
def processSensorData(sensorData, og, sm):
    ...
    for key in sorted(sensorData.keys()):
        ...
        if count == 1:
            ...
            matchedReading, confidence = sm.matchScan(
                estimatedReading, estMovingDist, estMovingTheta, count)
            prevRawMovingTheta = rawMovingTheta
            prevMatchedMovingTheta = getMovingTheta(
                matchedReading, xTrajectory, yTrajectory)
        #########   For Debug Only  #############
...
def compareGT(currentRawReading, prevRawReading, matchedReading, prevMatchedReading, gtReading, prevGtReading):
    print("true last pos x: " +
          str(prevGtReading['x']) + ", y: " + str(prevGtReading['y']))
    print("true curr pos x: " +
          str(gtReading['x']) + ", y: " + str(gtReading['y']))
    gtMoveX, gtMoveY = gtReading['x'] - \
        prevGtReading['x'], gtReading['y'] - prevGtReading['y']
    print("true move x: " + str(gtMoveX) + ", y: " + str(gtMoveY) + ", r: " + str(
        math.sqrt(gtMoveX ** 2 + gtMoveY ** 2)))

    print("Estd last pos x: " +
          str(prevMatchedReading['x']) + ", y: " + str(prevMatchedReading['y']))
    print("Estd curr pos x: " +
          str(matchedReading['x']) + ", y: " + str(matchedReading['y']))
    ...
    currMatchMinusRawMoveX = matchedReading['x'] - \
        prevMatchedReading['x'] - rawEstMoveX
    currMatchMinusRawMoveY = matchedReading['y'] - \
        prevMatchedReading['y'] - rawEstMoveY
    print(
